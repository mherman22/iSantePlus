...
public class IsanteplusLoginPageController {
    ...
    public String get(PageModel model, UiUtils ui, PageRequest pageRequest,
        ...
        if (Context.isAuthenticated()) {
            ...
            String redirectUrl = this.getStringSessionAttribute("_REFERENCE_APPLICATION_REDIRECT_URL_",
        ...
        if (sessionLocation != null && sessionLocation.hasTag("Login Location").booleanValue()) {
            ...
            try {
                ...
                pageRequest.getSession().setAttribute("_REFERENCE_APPLICATION_ERROR_MESSAGE_",
                        ui.message("referenceapplication.error.login.fail", new Object[0]));
            }
        } else if (sessionLocation == null) {
            pageRequest.getSession().setAttribute("_REFERENCE_APPLICATION_ERROR_MESSAGE_",
                    ui.message("referenceapplication.login.error.locationRequired", new Object[0]));
        } else {
            pageRequest.getSession().setAttribute("_REFERENCE_APPLICATION_ERROR_MESSAGE_", ui.message(
                    "referenceapplication.login.error.invalidLocation", new Object[] { sessionLocation.getName() }));
        }
