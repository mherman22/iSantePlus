SELECT p.patient_id
   ...
   MAX(ifnull(DATE(pdisp.dispensation_date),DATE(pdisp.visit_date))) 
   as min_visit_date FROM isanteplus.patient_dispensing pdisp
   WHERE  pdisp.voided <> 1  GROUP BY 1) B,
