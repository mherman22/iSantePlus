/*5 : Patients VIH+ sous ARV nouvellement mis sous CTX*/
SELECT p.patient_id
FROM isanteplus.patient_on_arv p,
     ...
     (SELECT psa.patient_id,
      ...
      FROM isanteplus.patient_status_arv psa WHERE DATE(psa.start_date)
...
AND DATE(ps.start_date) BETWEEN :startDate AND :endDate
