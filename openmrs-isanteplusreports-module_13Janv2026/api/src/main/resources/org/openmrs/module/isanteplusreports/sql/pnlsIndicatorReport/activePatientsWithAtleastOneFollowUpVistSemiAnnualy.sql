   ...
   isanteplus.patient_status_arv psa,
    (SELECT psar.patient_id,
      ...
      BETWEEN :startDate AND :endDate GROUP BY 1) B
	   ...
	   AND DATEDIFF (:endDate , pa.last_folowup_vist_date ) <= 180
       AND ((DATEDIFF(pa.last_folowup_vist_date,IFNULL(DATE(pa.second_last_folowup_vist_date), DATE(pa.first_vist_date))) BETWEEN 90 AND 179) OR
           (DATEDIFF (:endDate  , pa.last_folowup_vist_date ) > 90 AND DATEDIFF(pa.last_folowup_vist_date,IFNULL(DATE(pa.second_last_folowup_vist_date), DATE(pa.first_vist_date))) < 90 ));