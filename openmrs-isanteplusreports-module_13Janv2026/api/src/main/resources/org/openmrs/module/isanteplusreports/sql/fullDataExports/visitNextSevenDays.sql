...
WHERE pvi.next_visit_date between date(now()) and date_add(date(now()),interval 7 day) GROUP BY 1,3) B
...
AND pdisp.next_dispensation_date between date(now()) and date_add(date(now()),interval 7 day) GROUP BY 1,3) C
