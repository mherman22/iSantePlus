...
public class IsantePlusServiceImpl extends BaseOpenmrsService implements IsantePlusService {
     ...
     *
     * @param connection, this should only be passed on from
    ...
    /**
     * @param encounter, required for every form history since a every encounter is
     *                   submitted through a form
     * @param contextUp, defines whether the OpenMRS {@link Context} is accessible
    ...
    /**
     * @param extensions, of format {extensionId: true/false}
     */
    @Override
    ...
    public DataSet getDateStartedArv(Patient patient) {
        ...
        StringBuilder sqlQuery = new StringBuilder("select distinct s_arv.patient_id, s_arv.date_started_arv");
        sqlQuery.append(" FROM isanteplus_patient_arv s_arv");
    ...
    public List<IsantePlusObs> getViralLoadHistory(Patient patient) {
        // TESTS ORDERED = 1271
        List<IsantePlusObs> viralLoadHistory = new ArrayList<>();
        Integer labConceptId = 1271;
        Concept testsOrdered = Context.getConceptService().getConcept(labConceptId);
