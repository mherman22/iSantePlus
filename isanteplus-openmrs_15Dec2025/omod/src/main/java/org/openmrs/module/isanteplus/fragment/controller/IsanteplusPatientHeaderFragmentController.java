...
public class IsanteplusPatientHeaderFragmentController {
	...
	public void controller(FragmentConfiguration config, @SpringBean("emrApiProperties") EmrApiProperties emrApiProperties,
		...
		wrapper.getPatient().getActiveIdentifiers().forEach(identifier -> {
			patientIdentifierTypeList.add(identifier.getIdentifierType());
		});

		wrapper.getPatient().getActiveIdentifiers().forEach(patientIdentifier -> {
			if(patientIdentifier.getIdentifierType().getUuid().equals("e26ca279-8f57-44a5-9ed8-8cc16e90e559")) {
		...
		config.addAttribute("isFingerprint", isFingerprint);
