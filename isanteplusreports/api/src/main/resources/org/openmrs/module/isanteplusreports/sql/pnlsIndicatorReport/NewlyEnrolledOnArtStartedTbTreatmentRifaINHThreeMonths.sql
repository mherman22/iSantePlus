...
WHERE p.patient_id = pd.patient_id
  ...
  AND (p.birthdate <>'' AND p.birthdate is not null)
  AND p.date_started_arv IS NOT NULL
  AND TIMESTAMPDIFF(MONTH,p.date_started_arv,'2025-02-28') >= 3
  AND TIMESTAMPDIFF(MONTH,p.date_started_arv,'2025-02-28') < 4
  AND pd.drug_id  =78280
  AND pd.rx_or_prophy = 163768
  AND pd.visit_date = B.min_vist_date
  AND TIMESTAMPDIFF(MONTH,pd.visit_date,'2025-02-28') >= 3
  AND TIMESTAMPDIFF(MONTH,pd.visit_date,'2025-02-28') < 4
  AND p.voided <> 1;
*/